...
archiwum.app = Backbone.View.extend({
    ...
    yearClick: function (e) {
        ...
        Backbone.trigger('graduatesLoad', { IDY: $(e.target).attr('data-idy') });
    ...
    graduatesLoad: function (IDY) {
        ...
        self.model.fetch({
            ...
            var nextidy = yearsCollection.find(function (model) { return parseInt(model.get('Name')) == parseInt(thisyear.get('Name')) + 1; });
            if (typeof nextidy === 'undefined') {
                ...
                var nextyear = yearsCollection.find(function (model) { return model.get('YearId') == nextidy.get('YearId'); });
                var html = '<div class="col-md-1 col-sm-2 col-xs-2 year" data-idy="' + thisyear.get('YearId') + '"><div>' + thisyear.get('Name') + '</div></div><div class="items col-md-6 col-sm-4 col-xs-4"></div><div class="col-md-1 col-sm-2 col-xs-2 year nextyear" data-idy="' + nextyear.get('YearId') + '"><div data-idy="' + nextyear.get('YearId') + '">' + nextyear.get('Name') + '</div></div>';
            }
